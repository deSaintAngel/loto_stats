...
# x_train = tf.reshape(x_train,(num_samples, series_size, -1))
...
# model = define_autoencoder_model(input_shape)
...
es = EarlyStopping(monitor='loss', mode='max', verbose=0, patience=500)
reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.95, patience=20, min_lr=0.0000000000001)
save_model = SaveModelOnThreshold(filepath='./data/model_ckpt/model_checkpoint.h5', monitor='loss', threshold=0.001)

# Train the model
model.fit(x_train, y_train, batch_size=1000, epochs=num_epochs, verbose=0, callbacks=[progress_callback, reduce_lr])
