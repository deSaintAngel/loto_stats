...
def define_autoencoder_model(window_length, number_of_features, nb_label_feature):
    model = Sequential()
    model.add(LSTM(32, input_shape=(window_length, number_of_features), return_sequences=True))
    model.add(Dropout(0.2))  # Ajout de Dropout avec un taux de 0.2
    model.add(LSTM(32, return_sequences=True))
    model.add(Dropout(0.2))  # Ajout de Dropout avec un taux de 0.2
    model.add(LSTM(32, return_sequences=True))
    model.add(Dropout(0.2))  # Ajout de Dropout avec un taux de 0.2

    # Ajout des trois couches LSTM supplémentaires
    model.add(LSTM(32, return_sequences=True))
    model.add(Dropout(0.2))  # Ajout de Dropout avec un taux de 0.2
    model.add(LSTM(32, return_sequences=True))
    model.add(Dropout(0.2))  # Ajout de Dropout avec un taux de 0.2
...
model = define_autoencoder_model(series_size , number_of_features, np.shape(y_train)[-1])
#%% train the model
model.summary()
...
reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.9, patience=5)
save_model = SaveModelOnThreshold(filepath='./data/model_ckpt/model_checkpoint.h5', monitor='loss', threshold=0.001)

# Train the model
model.fit(x_train, y_train, batch_size=32, epochs=num_epochs, verbose=0, callbacks=[progress_callback, reduce_lr])
